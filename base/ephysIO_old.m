...
function [array,xdiff,xunit,yunit,names,notes,clist,saved] = ...
  ...
  if (nargin <= 1)
    ...
    elseif strcmpi(filename(end-4:end),'.tdms')
      if exist('TDMS_getStruct')
        [array,xdiff,xunit,yunit,names,notes,clist] = TDMSload (filename,ch);
      else
         error('the required helper function TDMS_getStruct.m cannot be found')
      end
    end
    if ~exist('clist','var')
      clist = cell(1);
...
function [array,xdiff,xunit,yunit,names,notes,recChNames] = TDMSload (filename,ch)

  % LabVIEW Signal Express
  % Technical Data Management solution (TDMS) binary files
  DataSet = TDMS_getStruct(filename);
  DataSet = rmfield(DataSet,'Props');
  DataSet = struct2cell(DataSet);
  y = cell(1,numel(DataSet));
  % Loop through data set
  for i=1:numel(DataSet)
    ChannelGroup = DataSet{i};
    ChannelGroup = rmfield(ChannelGroup,{'name','Props'});
    if i==1
      recChNames = fieldnames(ChannelGroup);
      nChannels = numel(recChNames);
      % Evaluate the recording channels
      fprintf('Number of recording channels: %d\n',nChannels);
      for j = 1:nChannels
      ...
      ChannelGroup = struct2cell(ChannelGroup);
      if isfield(ChannelGroup{ch}.Props,'wf_xunit_string')
        xunit = ChannelGroup{ch}.Props.wf_xunit_string;
      else
        xunit = 's';
        warning('wf_xunit_string is not defined')
      end
      if isfield(ChannelGroup{ch}.Props,'wf_start_offset')
        xOffset = ChannelGroup{ch}.Props.wf_start_offset;
      else
        xOffset = 0;
        warning('wf_start_offset is not defined')
      end
      if isfield(ChannelGroup{ch}.Props,'wf_increment')
        xdiff = ChannelGroup{ch}.Props.wf_increment;
      else
        xdiff = 1;
        warning('wf_increment is not defined')
      end
      if isfield(ChannelGroup{ch}.Props,'unit_string')
        yunit = ChannelGroup{ch}.Props.unit_string;
      else
        yunit = '';
        warning('unit_string is not defined')
      end
    else
      ChannelGroup = struct2cell(ChannelGroup);
      if isfield(ChannelGroup{ch}.Props,'wf_xunit_string')
        if ~strcmp(ChannelGroup{ch}.Props.wf_xunit_string,xunit)
          error('inconsistent wf_xunit_string across data set')
        end
      end
      if isfield(ChannelGroup{ch}.Props,'wf_start_offset')
        if ChannelGroup{ch}.Props.wf_start_offset ~= xOffset
          warning('unequal wf_start_offset across data set')
        end
      end
      if isfield(ChannelGroup{ch}.Props,'wf_increment')
        if ChannelGroup{ch}.Props.wf_increment ~= xdiff
          error('unequal wf_increment across data set')
        end
      end
      if isfield(ChannelGroup{ch}.Props,'unit_string')
        if ~strcmp(ChannelGroup{ch}.Props.unit_string,yunit)
          warning('inconsistent unit_string across data set')
        end
      end
    end
    if ~isempty(ChannelGroup{ch}.data)
      y{i} = ChannelGroup{ch}.data(:);
    else
      y{i} = 0;
    end
  end
  y = cells2mat(y);
  [nPoints, nWaves] = size(y);

  % Cleanup unit strings
  xunit = deblank(regexprep(xunit,'''',''));
  yunit = deblank(regexprep(yunit,'''',''));
